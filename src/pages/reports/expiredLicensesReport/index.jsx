  ...
  return (
    <Box m={ "20px" }>
      <Header
        ...
        subtitle={ t("reports.driversWithExpiredLicenses") }
      />
      { search && (
        <LoadingButton
          loading={ loading }
      ...
      { search && (
        <div>
          ...
          >
            { `${t("reports.startDate")}: ${info.startDate.format("DD/MM/YYYY")}` }
          </Typography>
          <Typography
            variant="h4"
            sx={ { mt: "20px", mb: "10px" } }
            color={ colors.gray[100] }
          >
            { `${t("reports.endDate")}: ${info.endDate.format("DD/MM/YYYY")}` }
          </Typography>
          <Box
            sx={ {
              height: "80vh",
              ...
              components={ {
                Toolbar: () => (
                  <TableToolbar
                    columns={ columns }
                    rows={ info.rows }
                    fileName={ t("reports.driversWithExpiredLicenses") }
                  />
                ),
              } }
            />
