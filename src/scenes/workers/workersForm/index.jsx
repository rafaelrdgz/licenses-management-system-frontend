...
import {createWorker, getWorkerById, updateWorker} from "../../../apis/WorkerAPI.js";
...
function WorkersForm() {
  ...
  const [info, setInfo] = useState({
    id: "", name: "", lastNames: "", password: "", email: "", role: ""
  });
  ...
  const initialValues = {
    id: info.id, name: info.name, password: info.password, email: info.email, lastNames: info.lastNames, role: info.role
  };
  ...
  const checkoutSchema = yup.object().shape({
    id: yup
      ...
      .test('is-valid-id', 'El número de indentificación no es válido', isValidIdDate), name: yup
      .string()
      .required("El nombre es requerido")
      .matches(/^[a-zA-ZÁÉÍÓÚáéíóú ]+$/, "El nombre no debe contener números ni caracteres especiales")
      .min(3, "El nombre debe tener al menos 3 caracteres")
      .max(25, "El nombre debe tener menos de 25 caracteres"), lastNames: yup
      .string()
      ...
      .max(50, "Los apellidos deben tener menos de 50 caracteres"), email: yup
      .string()
      .email("El correo debe ser un correo válido")
      .required("El correo es requerido")
      .matches(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/, "Correo no válido"), password: yup
      .string()
      .required("La contraseña es requerida")
      .min(8, "La contraseña debe tener al menos 8 caracteres")
      .max(20, "La contraseña debe tener menos de 20 caracteres"), role: yup
      .string()
  ...
  return (<Box m="20px">
    <Header title={"TRABAJADOR " + info.id} subtitle={editing ? 'Editar trabajador' : 'Insertar nuevo trabajador'}/>
    <Formik
      enableReinitialize
      validateOnMount
      initialValues={info}
      ...
      {({
        ...
        >
          ...
          <FormControl variant="filled" sx={{gridColumn: "span 2"}}>
            ...
            {touched.role && errors.role && (<FormHelperText sx={{color: '#f44336'}}>{errors.role}</FormHelperText> // Aquí se muestra el mensaje de error
            )}
          </FormControl>
        ...
        >
          <Button type="text"
                  ...
                  onClick={() => {
                    console.log(errors);
                    if (Object.keys(errors).length === 0 || (editing && "id" in errors && Object.keys(errors).length === 1)) handleFormSubmit(values);
                  }}>
            Guardar
          </Button>
        </Box>
